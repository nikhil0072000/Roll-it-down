...
void DoRoll_float(float3 vertexPos, out float3 result)
{
    ...
    if (_RollCenterPosX > vertexPos.x)
    {
        ...
        float incrementalRadius = Remap(0, _XLength, 0, 2, vertexPos.x) + vertexPos.z; // vertex.z ile toplamamızın sebebi x'leri aynı olan vertexlerin radiyuslarının farklı olması
        vertexPos = centerPoint + float3(cos(angleRadian), 0, sin(angleRadian)) * incrementalRadius;

        // Max radiyusu bulup öteliyoruz
        float maxRadius = Remap(0, _XLength, 0, 2, _RollCenterPosX);
